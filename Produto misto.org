:PROPERTIES:
:ID: 3D2EF035-0B4C-440F-B103-60AF81A90EB3
:END:
#+title: Produto misto
#+filetags: :M:
...
\[
  ...
  \end{vmatrix}
\]
As seguintes propriedades do produto misto decorrem principalmente das propriedades dos [[id:4090FB4D-4B9F-40AF-B335-7E88A513FE5B][determinantes]]. 1. O produto misto \((\vec{u},\vec{v},\vec{w})\) muda de sinal ao trocarmos a posição de dois vetores. 2. \((\vec{u} + \vec{x},\vec{v},\vec{w}) = (\vec{u},\vec{v},\vec{w}) + (\vec{x},\vec{v},\vec{w})\) 3. \((\alpha\vec{u},\vec{v},\vec{w})=(\vec{u},\alpha\vec{v},\vec{w})=(\vec{u},\vec{v},\alpha\vec{w})=\alpha(\vec{u},\vec{v},\vec{w})\) 4. \((\vec{u},\vec{v},\vec{w})=0\) se, e somente se, os três vetores forem *coplanares*.
...
[[file:volumeparalelepipedoprodutomisto.png.org][volumeparalelepipedoprodutomisto.png]]

A área da base do paralelepípedo é dada por \(|\vec{v} \times \vec{w}|\), como definido nos [[file:Produto vetorial#Módulo.org][produtos vetoriais]]. Sendo \(\theta\) o ângulo entre \(\vec{u}\) e \(|\vec{v} \times \vec{w}|\), a altura do paralelepípedo é dada por:
\[
  h=|\vec{u}||\cos{\theta}|
\]
...
[[file:tetraedroprodutomisto.png.org][tetraedroprodutomisto.png]]
