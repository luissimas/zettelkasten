:PROPERTIES:
:ID: c0555ee1-2632-4ac7-9144-790c3a04412d
:END:
...
Um grafo orientado é um par \((V, A)\) onde: - \(V\) é um conjunto qualquer (de vértices). - \(A\) é uma relação em \(V\), ou seja, um subconjunto do produto cartesiano \(V \times V\) onde cada aresta é um *par ordenado* de vértices.
...
Uma outra representação possível é combinar as matrizes de entrada e de saída em uma única matriz \(M_{ik}\) cujos elementos são inteiros pertencentes ao conjunto \({-1, 0, 1}\). Nesse caso, o elemento \(M_{ik}\) é: - \(1\) se a aresta \(e_k\) entra no vértice \(v_i\) - \(-1\) se a aresta \(e_k\) sai do vértice \(v_i\) - \(0\) se a aresta \(e_k\) não incide no vértice \(v_i\)
