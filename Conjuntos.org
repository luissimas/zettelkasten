:PROPERTIES:
:ID: d59212f8-7822-45c5-a37a-88c3198645a3
:END:
#+title: Conjuntos
#+filetags: :A:
...
Cada objeto de um conjunto *pertence* ao dado conjunto. A relação de pertinência a um conjunto é denotada pelo símbolo \(\in\). - \(x \in A\) significa que o objeto \(x\) *é* um elemento do conjunto \(A\). - \(x \notin A\) significa que o objeto \(x\) *não é* um elemento do conjunto \(A\).
...
** Subconjunto
Um conjunto \(A\) é um *subconjunto* de \(B\) se todo elemento de \(A\) também é elemento de \(B\). Essa relação pode ser denotada por: - \(A \subseteq B\): \(A\) está contido em \(B\) - \(B \supseteq A\): \(B\) contém \(A\)

\(A\) não é subconjunto de \(B\) se pelo menos um elemento de \(A\) não pertence a \(B\), essa relação é denotada por \(A \not\subset B\).

Algumas propriedades importantes da contingência de conjuntos: 1. Todo conjunto é subconjunto do conjunto universo e contém o conjunto vazio: \(\varnothing \subseteq A \subseteq \mathbb{U}\) 2. Todo conjunto é subconjunto de si mesmo: \(A \subseteq A\) 3. Transitividade: se \(A \subseteq B\) e \(B \subseteq C\) então \(A \subseteq C\) 4. Igualdade: \(A = B\) se e somente se \(A \subseteq B\) e \(B \subseteq A\)
