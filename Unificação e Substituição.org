:PROPERTIES:
:ID: c3abd1a6-be83-4152-9872-bcaf33f6d48e
:END:
#+title: Unificação e Substituição
...
Existem dois tipos especiais de substituições: - *Substituição vazia* (\(\epsilon\)): é uma substituição que não contém nenhum elemento. - *Substituição /ground/*: é uma substituição na qual \(t_1,\dots,t_n\) são termos /ground/, ou seja, termos que não contém variáveis.
...
#+begin_quote
*/Unificação/*
Dado um conjunto \(S\) de expressões, e substituições \(k \in \mathbb{N}\) e \(\theta_k\). Iniciamos com \(k = 0\) e \(\theta_k = \{\}\). 1. Se \(|S\theta_k| = 1\), então terminamos, \(\theta_k\) é uma /mgu/ de \(S\). Caso contrário, determinamos o conjunto de diferenças \(D_k\) de \(S\theta_k\). 2. Se existe uma variável \(x\) e um termo \(t\) em \(D_k\) tal que \(x\) não ocorre em \(t\), então fazemos \(\theta_{k+1} = \theta_k{t/x}\), \(k=k+1\) e retornamos ao passo \(1\). Caso contrário, terminamos, e \(S\) não é unificável.
