:PROPERTIES:
:ID: 215DD1F0-B6D1-466E-8E8C-86FBD475984D
:END:
#+title: Cálculo proposicional
#+filetags: :Interpreta:Consequ:Classifica:Regras:Leis:

* Cálculo proposicional
O cálculo proposicional utiliza das propriedades da [[file:Consequência e equivalência lógica#Consequência lógica.org][consequência lógica]] para inferir e deduzir novos conhecimentos a partir de um conjunto de fatos conhecidos /a priori/.
...
#+begin_quote
Um argumento \(\alpha_1, \alpha_2, \alpha_3, \dots, \alpha_{n-1} \vdash \alpha_n\) é dito *válido* se e somente se \(\alpha_1, \alpha_2, \alpha_3, \dots, \alpha_{n-1} \vDash \alpha_n\), ou seja, se e somente \(\alpha_n\) é uma [[file:Consequência e equivalência lógica#Consequência lógica.org][consequência lógica]] das premissas.
...
\]
for uma [[file:Semântica da lógica proposicional#Classificação de fórmulas.org][tautologia]].
...
*** Método semântico
Esse método consiste na construção da tabela-verdade e análise das [[file:Semântica da lógica proposicional#Interpretação I.org][interpretações]]. Esse método é mecânico e conceitualmente simples, porém pode ser inviável por conta da quantidade de proposições na fórmula.
Para verificar a validade de um argumento \(\alpha_1, \alpha_2, \alpha_3, \dots, \alpha_{n-1} \vdash \alpha_n\) utilizando o método semântico, basta fazer a tabela-verdade com todas as fórmulas presentes e verificar se \(\left(\alpha_1 \land \alpha_2 \land \alpha_3 \land \dots \land \alpha_{n-1} \right) \implies \alpha_n\) é uma [[file:Semântica da lógica proposicional#Classificação de fórmulas.org][tautologia]].

*** Método sintático
Esse método representa a *essência do cálculo proposicional*. Para verificar a validade de um argumento utilizando o método sintático, deve-se desenvolver uma [[id:09D8C7AD-6D5A-49A2-AB65-3032452EB2EB][prova (ou dedução)]] para o dado argumento com base nas [[file:Cálculo proposicional#Regras de inferência.org][regras de inferência]] e [[file:Álgebra da lógica proposicional#Leis de equivalência.org][leis de equivalência]].
