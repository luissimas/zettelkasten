:PROPERTIES:
:ID: 15cbfe06-523b-4b50-b140-d2094528d890
:END:
#+title: Timing analysis

Timing analysis is used to calculate the total delay of a circuit that uses [[id:860d0638-e241-4bcc-b6a7-1fab58f2ed36][flip-flops]] considering its various propagation delays for each point in the circuit.
...
If that's true, then the signal have enough time to keep output stable, otherwise the circuit has a hold time violation.
...
#+attr_org: :width 400
[[file:attachments/timinganalysisexample.png]]
...
* Clock skew
:PROPERTIES:
:ID:       56d71442-dd13-4138-9d72-606278b61aab
:END:
Sometimes we have a delay in the arrival of clock in only one point of the circuit, we call that delay time \(t_{skew}\). The clock skew has the effect of reducing the delay of that part of the circuit, because it provides additional time before data is loaded into the flip-flop. So we have the following:
