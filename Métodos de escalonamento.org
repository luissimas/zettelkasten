:PROPERTIES:
:ID: bb2f9c6e-f26d-486c-9556-50839b5a3846
:END:
#+title: Métodos de escalonamento
...
É possível manipular sistemas sem alterar o conjunto das soluções de algumas formas: 1. Trocar duas equações entre si 2. Multiplicar uma equação por um número não nulo 3. Substituir uma equação pela soma dela com uma outra equação, ou múltiplo de outra equação (mantendo a outra equação no sistema!)
...
Uma matriz é considerada na forma escalonada reduzida por linhas se: 1. É uma matriz na forma escalonada 2. O primeiro elemento não nulo de cada linha não nula (pivô) é 1 3. Na coluna do pivô ele é o único elemento não nulo
