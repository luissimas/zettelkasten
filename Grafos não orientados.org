:PROPERTIES:
:ID: 0AEC3218-E628-402A-9689-C1438FEAED83
:END:
#+title: Grafos não orientados

* Grafos não orientados
Um grafo não orientado é um par \((V, A)\) onde: - \(V\) é um conjunto qualquer (de vértices). - \(A\) é uma relação em \(V\), ou seja, um subconjunto do produto cartesiano \(V \times V\) onde cada aresta é um *par não ordenado* de vértices.
