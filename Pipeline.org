:PROPERTIES:
:ID: F44B70F4-817D-4DFB-B98F-ADD4093CC1C8
:END:
#+title: Pipeline
#+filetags: :RISC:Datapath:

* Pipeline
É uma técnica de implementação [[file:Unidade de controle#Datapath multiciclo.org][multiciclo]] que permite um melhor aproveitamento de cada unidade funcional do processador. Ao invés de executar as instruções completas sequencialmente, o pipeline permite executar múltiplas tarefas simultaneamente usando *recursos diferentes.*
O pipelining não melhora a /latência/ de uma única instrução, mas sim de um conjunto de instruções executadas.
As arquiteturas [[file:CISC e RISC#RISC.org][RISC]] são propícias para a organização em /pipelining/, isso se deve à sua simplicidade de formato de instrução e à predominância de operandos do tipo registrador.
...
- ID (Instruction decode): Envio do /opcode/ à [[id:62249BF8-BCB1-4B00-B7A7-BBBB4DB00C9A][unidade de controle]] e leitura do valor dos registradores determinados na instrução (quando houver).
- EX (Execution): A execução da instrução varia de acordo com o tipo de instrução (nesse momento a unidade de controle já determinou de qual tipo é a instrução):
  - Se instrução load ou store: [[id:8BB1266E-7D32-4B40-ADCB-3062CF5F8146][ULA]] calcula o endereço de memória a ser acessado.
  - Se instrução R-type: [[id:8BB1266E-7D32-4B40-ADCB-3062CF5F8146][ULA]] executa a operação com os registradores lidos no estágio ID.
  - Se instrução branch: [[id:8BB1266E-7D32-4B40-ADCB-3062CF5F8146][ULA]] efetua o teste de condição de salto entre os registradores lidos no estágio ID.
...
- MEM (Memory access): Etapa de acesso à memória, diferentes instruções têm diferentes propósitos de acesso à memória:
  ...
  - Se instrução R-type: Armazena o resultado da [[id:8BB1266E-7D32-4B40-ADCB-3062CF5F8146][ULA]] em um registrador temporário.
- WB (Write back): Armazena no arquivo de registradores os resultados obtidos em estágios anteriores, esses resultados podem ser:
  ...
  - Resultado da [[id:8BB1266E-7D32-4B40-ADCB-3062CF5F8146][ULA]] em instruções R-type.
...
[[file:sinaisdecontrolepipeline.png.org][sinaisdecontrolepipeline.png]]
...
[[file:pipeline.pdf.org][pipeline.pdf]]
