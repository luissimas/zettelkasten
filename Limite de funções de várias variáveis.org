:PROPERTIES:
:ID: A82A23C6-CB32-461B-8D04-C4D8DA3759EA
:END:
#+title: Limite de funções de várias variáveis
...
#+caption: limite
[[file:../Attachments/CDS/limitevariasvariaveis.png.org][../Attachments/CDS/limitevariasvariaveis.png]]
...
#+begin_quote
Dada uma função \(f(x,y)\) tal que \(\lim_{(x,y) \to (x_0, y_0)}{f(x,y)}=L\), e uma curva \(\gamma\) tal que: - \(\gamma (t_0) = (x_0, y_0)\) - \(\gamma (t) \neq \gamma (t_0)\), \(\forall t \neq t_0\) - \(\gamma (t) = (x(t), y(t))\), sendo \(x\) e \(y\) contínuas em \(t_0\)
