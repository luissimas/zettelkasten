:PROPERTIES:
:ID: 933c0eca-6590-4d6a-abc5-0c37f330100c
:END:
#+title: Álgebra da lógica proposicional
#+filetags: :Conectivos:Equival:

* Álgebra da lógica proposicional
A álgebra da lógica proposicional especifica como os elementos podem ser manipulados em fórmulas. Essas manipulações permitem que sejam geradas fórmulas [[file:Consequência e equivalência lógica#Equivalência lógica.org][equivalentes]] a outras através de simplificações das expressões lógicas.
...
#+begin_quote
O *dual* de uma fórmula definida em termos de símbolos atômicos \((p, q, \dots)\), símbolos de verdade \((V, F)\) e dos conectivos lógicos de conjunção \((\land)\) e disjunção \((\lor)\) é obtido substituindo-se *todas as ocorrências* de: - \(V\) por \(F\) e vice-versa - \(\land\) por \(\lor\) e vice-versa
...
#+begin_quote
Para toda fórmula \(\beta\) da [[file:Lógica Proposicional.org][Lógica Proposicional]] existe uma fórmula \(\alpha\) tanto na /FNC/ quanto na /FND/ que é equivalente a \(\beta\), ou seja: \(\alpha \equiv \beta\).
...
- *Cláusulas (C)*: uma cláusula é uma [[file:Sintaxe da lógica proposicional#Conectivos operadores lógicos.org][disjunção]] de literais \(L_1 \lor L_2 \lor \dots \lor L_n\), onde \(n \geq 0\) é o tamanho da cláusula.
...
- *Cláusulas duais (C)*: uma cláusula dual é uma [[file:Sintaxe da lógica proposicional#Conectivos operadores lógicos.org][conjunção]] de literais \(L_1 \land L_2 \land \dots \land L_n\), onde \(n \geq 0\) é o tamanho da cláusula.
